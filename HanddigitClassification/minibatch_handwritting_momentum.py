...
def initialize_parameters_deep(layer_dims): #layer_dims is a list containing the number of hidden units in each layer.
	...
	for i in range(1,len(layer_dims)):
		parameters["W"+str(i)] = np.random.randn(layer_dims[i],layer_dims[i-1])*1/np.sqrt(layer_dims[i-1])		
...
def L_model_forward(X, parameters):
	#numbr of layers
	l = len(parameters)//4 #parameter is a dictionary with 2 parameters(W,b,vdW,vdb) for each layer, so this works	
	A_prev = X
	caches = []
	i=0
...
def update_parameters(parameters, grads, learning_rate,t):
	...
	for l in range(L):
		#implementation of momentum without bias correction, no RMS prop because RMS prop giving 0/0 as input has numerous 0s
		parameters["vdW" + str(l+1)] = beta * parameters["vdW" + str(l+1)] + (1 - beta) * grads["dW" + str(l+1)]
		parameters["vdb" + str(l+1)] = beta * parameters["vdb" + str(l+1)] + (1 - beta) * grads["db" + str(l+1)]

		#parameters["vdW" + str(l+1)] = parameters["vdW" + str(l+1)]/(1-beta**(t+1))
		#parameters["vdb" + str(l+1)] = parameters["vdb" + str(l+1)]/(1-beta**(t+1))

		parameters["W" + str(l+1)] = parameters["W" + str(l+1)] - learning_rate * parameters["vdW" + str(l+1)] - learning_rate * lam * parameters["W" + str(l+1)]
		parameters["b" + str(l+1)] = parameters["b" + str(l+1)] - learning_rate * parameters["vdb" + str(l+1)]
...
def L_layer_model(X_train, Y_train, layers_dims, learning_rate = 0.5, num_epochs = 100,batch_size =64, print_cost=False):
	costs = []
...
diff_mtx = pred_train - Y_test #this is wrong as Y_test is not one hot encoded 
diff_mtx = diff_mtx[np.where(diff_mtx == 0)]
print ('Testing Accuracy: = '+ str(float(diff_mtx.size)/pred_train.size * 100)+'%')

#save parameters
np.save('parameters_momentum.npy', parameters) 
