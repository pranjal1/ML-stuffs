...
def plot_decision_boundary(X,Y,parameters):
	...
	Z = predict(X,parameters)
	Z = Z.reshape(1,Z.shape[0])

	fig = plt.figure()
	cols = color_set(Z[0,:])
	plt.scatter(xx, yy,c=cols,s=20, cmap=plt.cm.Spectral)
	plt.scatter(X_train[0,:], X_train[1,:],c=Y_train[0,:],s=10, cmap=plt.cm.Spectral)
	plt.xlim(xx.min(), xx.max())
	plt.ylim(yy.min(), yy.max())
	fig.savefig('multiclass_42_2_neurons1.png')
...
layers_dims = [X_train.shape[0],2,y_encoded.shape[0]] 
